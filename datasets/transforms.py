...
class GaussianBlur(object):
    """
    ✅ FIX: 统一成 (img, target) 接口，避免放进 Compose 时崩溃
    """
...
class RandomHorizontalFlip(object):
    ...
    def __call__(self, img, target):
        if random.random() < self.p:
            img = F.hflip(img)
            w, h = img.size
            # bbox is xyxy in absolute pixels here
            target['bbox'] = (
                target['bbox'][[2, 1, 0, 3]] * torch.as_tensor([-1, 1, -1, 1]) +
                torch.as_tensor([w, 0, w, 0])
            )
            target['phrase'] = (
                target['phrase']
                .replace('right', '*&^special^&*')
                .replace('left', 'right')
...
class RandomResize(object):
    def __init__(self, sizes, resize_long_side=True, record_resize_info=False):
        ...
        self.choose_size = max if resize_long_side else min
...
class RandomSizeCrop(object):
    ...
    def __call__(self, img, target):
        for _ in range(self.max_cnt):
            ...
            cropped_box = torch.min(
                cropped_box.reshape(-1, 2, 2),
                torch.as_tensor([tw, th], dtype=torch.float32)
            )
...
class NormalizeAndPad(object):
    """
    ✅ FIX:
    - mask 语义：True=padding(无效), False=有效
    - padding=False 时不允许平移（否则 bbox 会被错位平移）
    """
    def __init__(self, mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225], size=768,
    ...
    def __call__(self, img, target):
        ...
        if dw < 0 or dh < 0:
            # 如果出现输入比 size 大，说明前面 resize 配置不一致；这里直接报错更安全
            raise ValueError(f"NormalizeAndPad size={self.size} is smaller than input (h={h}, w={w}).")

        # padding=False 时不能随机平移/居中平移，否则 bbox 会错
        if not self.padding:
            top = left = 0
        else:
        ...
        if self.padding:
            out_img = torch.zeros((3, self.size, self.size), dtype=img.dtype)
            out_img[:, top:top + h, left:left + w] = img

            # True=padding区域(无效)，False=有效区域
            out_mask = torch.ones((self.size, self.size), dtype=torch.bool)
            out_mask[top:top + h, left:left + w] = False
            target['mask'] = out_mask
        else:
            out_img = img
            # 建议给一个全 False 的 mask，方便下游统一处理（可选）
            target['mask'] = torch.zeros((h, w), dtype=torch.bool)

        # shift bbox in pixel space (xyxy)
        box = target['bbox']
        box = box.clone()  # 避免原地改动导致潜在引用问题
