...
train_transforms = [
    dict(
        type='RandomSelect',
        transforms1=dict(type='RandomResize', sizes=[448, 480, 512, 544, 576, 608, 640, 768]),
        transforms2=dict(
            type='Compose',
            transforms=[
                ...
                dict(type='RandomResize', sizes=[448, 480, 512, 544, 576, 608, 640, 768])
            ],
        ),
        p=0.5
    ),
    ...
    dict(type='NormalizeAndPad', size=768, aug_translate=True)
]

test_transforms = [
    dict(type='RandomResize', sizes=[768], record_resize_info=True),
    dict(type='ToTensor', keys=[]),
    dict(type='NormalizeAndPad', size=768, center_place=True)
]